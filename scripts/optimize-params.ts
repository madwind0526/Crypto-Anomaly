...
function openPos(price: number): Pos { return { entry: price, high: price, hold: 0 }; }
function closePos(pos: Pos, exitPrice: number, cash: number) {
  return cash * (1 + (exitPrice - pos.entry) / pos.entry);
}
interface BtResult { returnRate: number; trades: number; }
...
function backtestA(
  ...
  let cash = 1.0; let pos: Pos | null = null; let trades = 0;
  for (let i = 52; i < candles.length; i++) {
    ...
    if (pos) {
      ...
      if (price <= stop || fade || pos.hold >= maxHold) {
        cash = closePos(pos, price <= stop ? stop : price, cash);
        pos = null; trades++;
      } else { pos.hold++; }
    } else {
  ...
  if (pos) cash = closePos(pos, candles.at(-1)!.close, cash);
  return { returnRate: cash - 1, trades };
}
...
function bestCombo<P extends object>(
  ...
  let bestReturn = -Infinity; let bestP = defaults; let bestR: BtResult = { returnRate: 0, trades: 0 };
  ...
  if (bestReturn === -Infinity) { bestP = defaults; bestR = { returnRate: 0, trades: 0 }; }
...
export async function runOptimization(
  ...
  for (const market of marketNames) {
    ...
    if (candles.length < 60) {
      console.log(`  [opt] ${market.replace("KRW-", "").padEnd(10)} skip (only ${candles.length} candles yesterday)`);
      continue;
    }
    ...
    const tag = (r: BtResult) => `${(r.returnRate * 100).toFixed(1)}%(${r.trades}t)`;
    console.log(`  [opt] ${market.replace("KRW-", "").padEnd(10)} A:${tag(rA)}  B:${tag(rB)}  C:${tag(rC)}  D:${tag(rD)}`);

    params[market] = {
      "momentum":   { ...pA, trades: rA.trades, returnRate: rA.returnRate },
      "range-grid": { ...pB, trades: rB.trades, returnRate: rB.returnRate },
      "arbitrage":  { ...pC, trades: rC.trades, returnRate: rC.returnRate },
      "anomaly":    { ...pD, trades: rD.trades, returnRate: rD.returnRate },
    };
  }
